...
fn parse(input: String) -> Vec<(usize, Vec<usize>)> {
    input
        ...
        .map(|line| {
            ...
            (parts.0.parse().unwrap(),parts.1.split(" ").map(|num| num.parse().unwrap()).collect())
        })
        .collect()
}
...
fn find_target_2(target: usize, mut nums:Vec<usize>) -> bool{
    ...
    find_target_2(target, add_option) || find_target_2(target, mul_option) || find_target_2(target, con_option)
}
